...
from esutils import admins, commands, configurations, exceptions, tools
import es
...
def _action_transformer(value):
    if value in modules.Action.actions:
        return value
    raise commands.ArgumentValueError()
    
_action = modules.Parameter("action", "The basename of an action")
_action.set_transformer(_action_transformer)

def _loaded_module_transformer(value):
...
def _unloaded_module_transformer(value):
...
def _help_player(informer, action=None, user_id=None, by_menu=False):
...
_help = modules.Action(_help_player, "help",  
                       ...
                       "action or overview the whole admin system", False)
_help.set_parameter(_action, None)
...
_load.set_parameter(_unloaded_module)
...
_unload.set_parameter(_loaded_module)
...
def player_activate(event_var):
    if admins.is_user_authorised(event_var["es_steamid"]):
        es.tell(int(event_var["userid"]), 
                "Welcome %s. Type %shelp in console or %shelp in chat for "
                "more information." 
