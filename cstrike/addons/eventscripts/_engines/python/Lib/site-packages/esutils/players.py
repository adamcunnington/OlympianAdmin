...
def _get_weapon(player):
    ...
    for index in weapons.weapon_type_by_name(weapon).indexes:
        ownership = es.getindexprop(index, "CBaseCombatWeapon.m_hOwner")
        if ownership == player.handle:
            return weapons.Weapon(index)
            
def _has_c4(player):
    for index in weapons.weapon_type_by_name("weapon_c4").indexes:
        ownership = es.getindexprop(index, "CBaseCombatWeapon.m_hOwner")
        if ownership == player.handle:
            return True
    return False
...
def _is_burning(user_ID):
    return bool(_get_flame_indexes(user_ID))
...
class Player(object):
    ...
    burning = _player_property(_is_burning)
    can_buy = _player_property(_PlayerEntity("CCSPlayer.m_bInBuyZone").get, 
                               _modulus_transformer)
    c4 = _player_property(_has_c4, pass_instance_get=True)
    can_plant = _player_property(_PlayerEntity("CCSPlayer.m_bInBombZone").get, 
                                 _modulus_transformer)
    ...
    flash_alpha = _player_property(_flash_alpha.get, setter=_flash_alpha.set, 
                                   set_transformer=int)
    ...
    melee = _player_property(_get_melee, pass_instance_get=True)
    ...
    observer = _player_property(_PlayerAttribute("isobserver").get, 
    ...
    weapon = _player_property(_get_weapon, pass_instance_get=True, 
    ...
    def __init__(self, user_ID):
        ...
        self.auto_update_attributes = False
    ...
    def extinguish(self):
        ...
        """
        for index in _get_flame_indexes(self.user_ID):
            es.setindexprop(index, "CEntityFlame.m_flLifetime", 0)
            
    def get_ammo(self, weapon_name):
        """Return the ammo amount in the player's weapon.
        
        Arguments:
        weapon_name - the full name of the weapon to find the amount of ammo 
        of.
        
        """
        return es.getplayerprop(self.user_ID, 
                                weapons.weapon_type_by_name(weapon_name))
    ...
    def push(self, horizontal, vertical, vertical_override=False):
        ...
        """
        x, y, z = _get_view_vector(self.user_ID)
        horizontal_x = horizontal * x
        horizontal_y = horizontal * y
    ...
    def remove_weapon(self, weapon_name):
        """Forcefully remove a specific weapon from the player.
        
        Arguments:
        weapon_name - the full name of the weapon to be removed.
        
        """
        for index in weapons.weapon_type_by_name(weapon_name).indexes:
            ownership = es.getindexprop(self.user_ID, 
                                        "CBaseCombatWeapon.m_hOwner")
            if ownership == self.handle:
                es.server.insertcmd("es_xremove %s" % index)
                return
        raise PlayersError("User ID, %s does not own %s" 
                           %(self.user_ID, weapon_name))
                           
    def set_ammo(self, weapon_name, value):
        """Set the ammo of the player's weapon.
        
        Arguments:
        weapon_name - the full name of the weapon to have it's ammo set.
        value - the new value to set the ammo at.
        
        """
        weapon = weapons.weapon_type_by_name(weapon_name)
        es.setplayerprop(self.user_ID, weapon.ammo_property, value)
