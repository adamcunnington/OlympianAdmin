...
class _Command(object):
    ...
    @classmethod
    def _inform(cls, message, user_ID):
        if user_ID is None:
    ...
    @classmethod
    def _invalid_argument(cls, command_name, parameter, arg, user_ID=None):
        cls._inform("Invalid Argument, %s is an invalid value for parameter, "
                    "%s." %(arg, parameter.basename), user_ID)

    @classmethod
    def _invalid_syntax(cls, command_name, parameters, args, user_ID=None):
        cls._inform("Invalid Syntax, parameter syntax: %s %s." %(
                    command_name, cls._names[command_name].callback.syntax),
                    user_ID)

    @classmethod
    def _no_auth(cls, command_name, user_ID=None):
        cls._inform("Authorisation Error, you do not have the required "
...
class ClientConsoleCommand(_ClientCommand):
    ...
    def _callback(cls):
        ...
        try:
            ...
            command.invalid_syntax_callback(name, command.callback.parameters,
                                            args, user_ID=user_ID)
        except ArgumentValueError, (parameter, arg):
            command.invalid_argument_callback(name, parameter, arg,
                                              user_ID=user_ID)
        else:
...
class ServerCommand(_Command):
    ...
    @classmethod
    def _disabled(cls, command_name):
        cls.INFORMER("Command Disabled, %s is no longer active." %
                     command_name)
