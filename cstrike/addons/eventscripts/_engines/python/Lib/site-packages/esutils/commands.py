...
class _Command(object):   
    ...
    def _get_transformed_args(cls, callback, args, user_ID=None):
        ...
        if not (total_parameters >= total_args >= 
                total_parameters - len(callback.defaults)):
        ...
        for index in xrange(total_args, total_parameters):
            parameter = callback.parameters[index]
            default = callback.defaults[parameter]
            if parameter.transformer is not None:
                # Declare variable for readable line length.
                arg = parameter.transformer(default, user_ID=user_ID)
                transformed_args.append(arg)
            else:
                transformed_args.append(default)
    ...
    def _invalid_syntax(self, command, user_ID=None):
...
class Callback(object):
    ...
    def set_parameter(self, parameter, default_value=ABSENT):
        """Set a parameter for the callback.
        
        Arguments:
        parameter - the parameter to register for the callback.
    ...
    def _callback(cls):
        ...
        if (command.requires_auth and 
            not admins.is_user_authorised(es.getplayersteamid(user_ID), 
                                          command.permission)):
            command.no_auth_callback(user_ID)
            return
        ...
        try:
            ...
            command.invalid_syntax_callback(name, user_ID=user_ID)
        except ArgumentValueError, (parameter, value):
            command.invalid_argument_callback(parameter, value, 
                                              user_ID=user_ID)
        else:
            command.callback.callback(command.INFORMER, user_ID=user_ID, *args)
...
class ServerCommand(_Command):
    ...
    def _callback(cls):
        ...
        try:
            ...
            command.invalid_syntax_callback(*args[0])
        except ArgumentValueError, (parameter, value):
            command.invalid_argument_callback(parameter, value)
        else:
