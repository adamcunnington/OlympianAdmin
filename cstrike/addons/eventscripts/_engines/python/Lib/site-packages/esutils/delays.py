...
"""Provides optimised methods which enable a user to delay the execution of
...
class Delay(object):
    ...
    def _set_interval(self, value):
        now = time.time()
        new_call_time = self.call_time + (value - self._interval)
        if new_call_time < now:
            if not self.recurring:
                raise DelaysError("cannot set new call time in the past on a "
                                  "non-recurring delay")
                # Needs to be called right now but slight delay required to
                # ensure it's always caught by the tick listener.
                self.call_time = now + 0.5
        else:
            self.call_time = new_call_time
    ...
    def _tick(cls):
        ...
        if not delay.recurring:
            ...
            delay.call_time = now + delay._interval
    ...
    def start(self, interval, recurring=False, call_now=False):
        ...
        happen recurrently or just be a singular occurrence.
