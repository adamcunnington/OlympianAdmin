...
"""Provide a powerful API to allow the user to create & manage menus 
...
from esutils import admins, delays, messages
import es
...
class Menu(object):
    ...
    def _build(self, user_ID, page):
        ...
        if self.get_title is None:
            ...
            lines.append(self._validate_true_value(
                         self.get_description(user_ID)))
        if self.get_description is None:
            lines.append(self._description)
        else:
            lines.append(self._validate_true_value(self.get_title(user_ID)))
        ...
        for i in xrange(self._max_items):
            ...
            if index < total_items:
                ...
                if item.selectable:
                    ...
                    if (not item.requires_auth or 
                        admins.is_user_authorised(steam_ID, item.permission)):
                ...
                break
        lines.append(self._separator)
        if self.players[user_ID].history:
            number = "7"
    ...
    def _client_command_filter(cls, user_ID, args=None):
        ...
        if selection == 7:
            ...
            option = menu.items[((page - 1) * 6) + selection - 1]
            cls._process_option_selection(player, option)
        return True
    ...
    def send(self, user_ID, page=1, reset_navigation=True):
        ...
        if self.get_items is not None:
            self.items[:] = self.get_items(user_ID)
...
def close_popup(user_ID):
    """Effectively close a user's popup by cancelling the next resend. raise 
